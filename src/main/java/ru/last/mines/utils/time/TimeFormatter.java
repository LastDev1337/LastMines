...
public class TimeFormatter {
    ...
    public static String getDefaultView(String path) {
        ...
        for (int i = 0; i < segments.length; i++) {
            ...
            if (!current.has(segment)) break;

            YamlValue node = current.get(segment);
        ...
        return defaultsViews.get("all").asString("default");
    }
    ...
    public static String formatDefault(long millis, String formatType) {
        ...
        if ("clock".equalsIgnoreCase(formatType) || "simple".equalsIgnoreCase(formatType)) {
            ...
            if (hours > 0) sb.append(hours).append(" ").append(plural(hours, "час", "часа", "часов", "часа"));
            if (minutes > 0) sb.append(minutes).append(" ").append(plural(minutes, "минута", "минуты", "минут", "минуты"));
